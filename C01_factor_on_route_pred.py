...
def generate_data_for_reg():
    score = pd.read_csv('final_selected_route.csv')
    score = score[['route_id','zone_len','score']].copy()
    ...
    route_att = route_att.merge(score, on = ['route_id'])
    assert len(route_att) == len(score)
    ...
    col_X = ['total_n_pkg','total_planned_service_time','min_time_window_diff','avg_num_tra_sig','total_num_stops',
             'exe_cap_m3','if_high_qua_route','if_in_LA','if_in_CH','if_in_BO','weekends','before_7am','after_10am']
...
def run_linear_reg(data, col_X):
    ...
    table.to_csv('table/estimate_para_on_score_new.csv',index=False)
