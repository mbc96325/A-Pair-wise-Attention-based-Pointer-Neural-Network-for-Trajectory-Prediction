...
if __name__ == '__main__':
    ...
    model_name = 'pnt_net_with_asnn_att_rand_seq'
    ...
    decoder = Decoder(num_features_without_zone, HIDDEN_SIZE, num_zone_id_feature=len(zone_id_feature),
                      max_len=zone_len_max, local_view=LOCAL_VIEW, HIDDEN_SIZE_asudnn=HIDDEN_SIZE_asudnn)
    ptr_net = PointerNetwork(encoder, decoder, num_zone_id_feature=len(zone_id_feature), input_seq_dict=input_seq_dict,
                             max_zone_len=zone_len_max)
    ...
    GENERATE_SEQ_DIFF_FIRST_ZONE = True
    #################
    if ONLY_MODEL_APPLY:
    ...
    for epoch in range(0, EPOCHS + 1):
        ...
        if not ONLY_MODEL_APPLY:
            ...
            for batch_x, batch_y, batch_z, batch_route, batch_ini_travel_time, batch_index in trainloader:
                ...
                optimizer.zero_grad()
                # Forward
                out, loss, weights = ptr_net(batch_x, batch_y, batch_z, batch_route, batch_ini_travel_time,
                                             batch_str_info, zone_mean_dist, mask_prob=False, teacher_force=True,
                                             Print_pred=False, No_dyn_dist=True)
                # out, loss, weights = ptr_net(batch_x, batch_y, batch_z, batch_route, mask_prob=True, teacher_force=False, Print_pred = False)
        ...
        out, loss, weights = ptr_net(X_test_t, Y_test_t, zone_len_test_t, route_test_seq, init_dist_seq_test_t,
                                     str_info_np_test, zone_mean_dist,
                                     mask_prob=True, teacher_force=False, Print_pred=False, No_dyn_dist=True,
                                     model_apply=True, Calculate_loss=True, pattern_force = True)
        ...
        if ONLY_MODEL_APPLY:
            ...
            save_eval_first_zone_df.to_csv('save_eval_first_zone_df_' + model_name + '.csv',index=False)
            break
    if not ONLY_MODEL_APPLY:
        torch.save(ptr_net.state_dict(), model_name + '.pt')
        try:
            train_records_df = pd.DataFrame(train_records)
            train_records_df.to_csv('training_record_' + model_name + '.csv')
        except:
            with open("training_record_" + model_name + ".pkl", "wb") as f:
